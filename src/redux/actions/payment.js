...
export const get_payment_total = (shipping_id, coupon_name) => async dispatch => {
    ...
    try {
        const res = await axios.get(`http://127.0.0.1:8000/api/payment/get-payment-total?shipping_id=${shipping_id}&coupon_name=${coupon_name}`, config);
...
export const get_client_token = () => async dispatch => {
    ...
    try {                           
        ...
        const res= await axios.get("http://127.0.0.1:8000/api/payment/get-token", config);    
...
export const process_payment = (
    ...
    const config = {
        headers : {   
            'Accept': 'application/json',
            'Content-Type': 'application/json',   //ES CONTENT PORQUE ES UN METODO POST!!
            'Authorization': `JWT ${localStorage.getItem('access')}`
        }
    };
    ...
    try {
        ...
        const res = await axios.post("http://127.0.0.1:8000/api/payment/make-payment", body, config); 
