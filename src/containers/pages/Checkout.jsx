...
import DropIn from "braintree-web-drop-in-react";  //LO RECIEN INSTALADO PARA TOMAR EL TOKEN 11:23
import Loader from 'react-loader-spinner';
import { Circles } from 'react-loader-spinner'
import { countries } from "../../helpers/fixedCountries";
import ShippingForm from "../../components/checkout/ShippingForm";

        //ESTA PAGINA PARA PROTEGER RUTAS NO LOGUEADAS Y SETEAMOS EN EL APPS.JS ROUTES
const Checkout = ({
    isAuthenticated, 
    items, 
    update_item, 
 ...
 const apply_coupon = async e => {
    ...
    useEffect(()=>{
      get_client_token();
    }, [user]);                          //USER ES EL CRITERIO PARA MOSTRAR EL RESULTADO DE ESTE USE_EFFECT
                  ...
                  shipping.map((shipping_option, index) => (
                    <div key={index}>
                        ...
                        <label className="ml-4">
                            {shipping_option.name} - ${shipping_option.price} ({shipping_option.time_to_delibery})
                        </label>
                    </div>
                  ))
                 }    
