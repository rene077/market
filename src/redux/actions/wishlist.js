...
export const get_wishlist_items = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.get("http://127.0.0.1:8000/api/wishlist/wishlist-items", config);
...
export const add_wishlist_item = product_id => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post("http://127.0.0.1:8000/api/wishlist/add-item", body, config);
...
export const get_wishlist_item_total = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.get("http://127.0.0.1:8000/api/wishlist/get-item-total", config);
